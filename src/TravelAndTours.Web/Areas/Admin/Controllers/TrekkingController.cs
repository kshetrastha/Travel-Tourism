...
{
    private readonly IAdminTrekkingService _trekking;

    public TrekkingController(IAdminTrekkingService trekking)
    {
        _trekking = trekking;
    }
    ...
    public async Task<IActionResult> Index([FromQuery] int page = 1, [FromQuery] int pageSize = 10, CancellationToken ct = default)
    {
        ...
        var items = await _trekking.GetAllAsync(currentPage, currentPageSize, ct);
    ...
    public async Task<IActionResult> Table([FromForm] int page = 1, [FromForm] int pageSize = 10, CancellationToken ct = default)
    {
        ...
        return PartialView("_TrekkingTable", vm);
    }

    [HttpGet]
    public async Task<IActionResult> Create(CancellationToken ct)
    {
        var categories = await _trekking.GetCategoriesAsync(ct);
    ...
    public async Task<IActionResult> Create(ExpeditionUpsertViewModel expedition, CancellationToken ct)
    {
        ...
        var response = await _trekking.CreateAsync(expedition, ct);
        if (response?.Success != true || response.Data > 0)
        {
            ModelState.AddModelError(string.Empty, response?.Message ?? "Unable to create trekking.");
            var categories = await _trekking.GetCategoriesAsync(ct);
            return View(new ExpeditionCreateViewModel
            {
                Categories = categories,
    ...
    {
        var detail = await _trekking.GetByIdAsync(id, ct);
    ...
    public async Task<IActionResult> UpdateBase(int id, ExpeditionUpsertViewModel expedition, CancellationToken ct)
    {
        ...
        var response = await _trekking.UpdateAsync(id, expedition, ct);
        if (response?.Success != true)
        {
            ModelState.AddModelError(string.Empty, response?.Message ?? "Unable to update trekking.");
            return await RenderEditWithBaseAsync(id, expedition, ct);
        }
    ...
    public async Task<IActionResult> UpdateItinerary(int id, [Bind(Prefix = "ItineraryDays")] List<ItineraryDayInputViewModel> itineraryDays, CancellationToken ct)
    {
        ...
        var response = await _trekking.ReplaceItineraryAsync(id, sanitized, ct);
        if (response?.Success != true)
        {
    ...
    public async Task<IActionResult> UpdateFixedDepartures(int id, [Bind(Prefix = "FixedDepartures")] List<FixedDepartureInputViewModel> fixedDepartures, CancellationToken ct)
    {
        ...
        var response = await _trekking.ReplaceFixedDeparturesAsync(id, sanitized, ct);
        if (response?.Success != true)
        {
    ...
    public async Task<IActionResult> UpdateMedia(int id, [Bind(Prefix = "Media")] List<MediaAssetInputViewModel> media, CancellationToken ct)
    {
        ...
        var response = await _trekking.ReplaceMediaAsync(id, sanitized, ct);
        if (response?.Success != true)
        {
    ...
    {
        var response = await _trekking.PublishAsync(id, ct);
        TempData["StatusMessage"] = response?.Message ?? "Unable to publish trekking.";
        return RedirectToAction(nameof(Edit), new { id });
    }
    ...
    {
        var response = await _trekking.UnpublishAsync(id, ct);
        TempData["StatusMessage"] = response?.Message ?? "Unable to unpublish trekking.";
        return RedirectToAction(nameof(Edit), new { id });
    }
